...
var exporter = async function (config, db) {
  var self = this;
  self.config = config;
  self.db = db;
  console.log("exporter init ,url=" + config.providerUrl)
  self.web3 = new Web3(new Web3.providers.WebsocketProvider(config.providerUrl));
  self.contract = self.web3.eth.Contract(config.erc20ABI, config.tokenAddress);

  // Subscribe to Transfer events matching filter criteria
  self.contract.events.Transfer({ fromBlock: config.exportNewBlockStartFrom, toBlock: "latest" }, async (error, event) => {
    if (error) {
      console.log(error)
      return
    }

    console.log('Found incoming Transfer transaction \n');
  ...
  self.contract.events.Approval({ fromBlock: config.exportNewBlockStartFrom, toBlock: "latest" }, async (error, event) => {
    if (error) {
      console.log(error)
      return
    }
    console.log('Found incoming  Approval transaction \n');
    console.log('Token address is: ' + event.address)
    //TODO 
  });

  // var getPastEvents =  (constract, fromBlock, toBlock) => {
  //   console.log("exporter getPastEvents ")
  //   return  constract.getPastEvents('allEvents', {
  //     filter: {},
  //     fromBlock: fromBlock,
  //     toBlock: toBlock,
  //   }, (error, events) => { console.log(events); console.error(error) });
  // }

  // self.allEvents = await getPastEvents(self.contract, config.exportStartBlock, config.exportNewBlockStartFrom);
  // self.newEvents = await getPastEvents(self.contract, config.exportNewBlockStartFrom, "latest");
  // console.log("exporter map ="+self.allEvents)
  // self.allEvents.map((event) => {
